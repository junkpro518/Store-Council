...
## SaaS conversion — Phases P4 remainder + P5 (2026-06-12)

T018/T022/T024–T027 complete (T028 = operator launch checklist, wired into OPERATOR-CHECKLIST). Accounts: login-with-Salla implemented end-to-end and proven against a mock Salla accounts service — authorize redirect → code exchange → user/info → merchant→tenant mapping → hashed merchant session; the merchant dashboard is now fully multi-tenant (a tenant's chat verifiably lands in that tenant's history and nowhere else). UI: Salla sign-in button, token capture, plan & usage card with quota bars, platform-managed-keys mode (AI/credential cards hidden in SaaS), impersonation banner. Fleet admin: tenant table with plan override + lock/unlock, audited impersonation (audit_log, migration 002), confirm-PURGE deletion, 90-day retention purge in the worker (cascade verified to wipe everything), and monitoring counters (queue depth/age, failures, tenants by status, tokens today). Proven by `tests/accounts-fleet.ts` (16/16, first run) plus full regression of all six suites. Platform-level Salla credentials (SALLA_CLIENT_ID/SECRET) now serve all tenants — fixing a real gap where provisioned tenants had no credentials for token refresh.
