...
| [docs/OPERATOR-CHECKLIST.md](docs/OPERATOR-CHECKLIST.md) | Living checklist of every procedure the platform owner performs (setup, release gates, recurring ops, incidents) |
...
**Inside Salla, with @mentions.** The merchant dashboard can run **embedded inside the Salla dashboard**: Settings generates a signed embed link to set as the App URL in the Partners portal — it auto-logs the merchant in inside the iframe (framing is allowed for `*.salla.sa` only; the central panel is never embeddable). And in chat, typing **`@`** suggests managers — `@pricing سؤالي…` (by id, English or Arabic name) routes the question to that manager and switches the conversation there.
